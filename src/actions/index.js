...
export async function fetchJobApplicationsForCandidate(candidateID) {
  ...
  const result = await Applications.find({ candidateUserID: candidateID });
...
export async function fetchJobApplicationsForRecruiter(recruiterID) {
  await connectToDB();
  const result = await Applications.find({ recruiterUserID: recruiterID });
  return JSON.parse(JSON.stringify(result));
}
