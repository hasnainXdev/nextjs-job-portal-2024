...
const CommonForm = ({
  ...
  function renderInputByComponentType(getCurrentControl) {
    ...
    switch (getCurrentControl.componentType) {
      case "input":
        /**
         * Render an input field with the given props
         */
        content = (
        ...
        content = (
          <div className="relative flex items-center mt-8">
            <Input
              ...
              name={getCurrentControl.name}
              id={getCurrentControl.name}
              value={getCurrentControl[formData.name]}
              onChange={(e) =>
                setFormData({
                  ...formData,
              ...
              className="w-full rounded-md h-[60px] px-4 border bg-gray-100 outline-none drop-shadow-sm transition-all duration-200 ease-in-out focus:drop-shadow-lg focus-visible:outline-none focus-visible:ring-0"
            />
          </div>
        );
        break;
